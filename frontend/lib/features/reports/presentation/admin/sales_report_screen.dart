...
class _SalesReportScreenState extends State<SalesReportScreen> {
  ...
  void _handleExport() {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          'Exporting CSV for "$_selectedRange"…',
  ...
  // ✅ The entire content area is now a SingleChildScrollView so nothing
  // ever gets clipped — both cards in Row 1 can be as tall as they need.
  Widget _buildScrollableContent() {
    return LayoutBuilder(builder: (context, constraints) {
      // Ideal heights based on available space
      final row1H = constraints.maxHeight * 0.52;
      final row2H = constraints.maxHeight * 0.44;
          ...
          children: [
            ...
            SizedBox(
              ...
              child: Row(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  // ✅ BusinessPerformanceCard is inside a scrollable container
                  // so its donut/legend never gets clipped — it just scrolls
                  // within the card if it overflows.
                  Expanded(
...
// Wraps any child that might overflow with a scroll inside a styled card.
// ─────────────────────────────────────────────────────────────────────────────
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        // ✅ No background here — BusinessPerformanceCard has its own bg
      ...
      child: ClipRRect(
        ...
        child: SingleChildScrollView(
          // ✅ Vertical scroll inside the card so the donut + legend
          // are always reachable without breaking the outer layout.
          physics: const BouncingScrollPhysics(),
          child: child,
        ),
      ),
                  ...
                  child: LayoutBuilder(builder: (_, box) {
                    return Row(
                      ...
                      children: List.generate(values.length, (i) {
                        final ratio =
                            maxVal > 0 ? values[i] / maxVal : 0.0;
                        final isMax = values[i] == maxVal;
                        final barH = (box.maxHeight * ratio).clamp(
                            0.0, box.maxHeight - (isMax ? 20.0 : 0.0));
                        return Expanded(
                          child: Padding(
                            padding:
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // ✅ ClipRect prevents any child from painting outside this card
