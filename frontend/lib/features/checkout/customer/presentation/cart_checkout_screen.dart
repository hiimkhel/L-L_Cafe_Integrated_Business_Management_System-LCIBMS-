...
class _CartCheckoutScreenState extends State<CartCheckoutScreen> {
  ...
  Future<void> _confirmOrder() async {
    ...
    if (_phoneCtrl.text.trim().isEmpty) {
      _snack('Please enter your contact number.', error: true);
      return;
    }
    if (_isDelivery && _addressCtrl.text.trim().isEmpty) {
      _snack('Please enter a delivery address.', error: true);
      return;
    }
    if (_items.isEmpty) {
    ...
    if (!_isCash && _proofImageUrl == null) {
      //-----------------------------------------------------
      _snack('Please attach your payment proof.', error: true);
      return;
    }
        ...
        child: AnimatedContainer(
          ...
          child: Text(
            ...
            style: TextStyle(
              ...
              fontWeight: FontWeight.bold,
              letterSpacing: 1.2,
              color: active ? _kWhite : _kPrimary,
            ),
          ),
        ),
      ),
      ...
      child: Column(
        ...
        children: [
          ...
          if (!_isCash) ...[
            const SizedBox(height: 12),
            Material(
              color: _proofImageUrl != null ? _kPrimary : _kSecondary,
              borderRadius: BorderRadius.circular(14),
              child: InkWell(
                onTap: _isUploading ? null : _pickAndUploadProof,
                borderRadius: BorderRadius.circular(14),
                splashColor: _kWhite.withOpacity(0.1),
                highlightColor: _kWhite.withOpacity(0.05),
                child: Container(
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          Text(
                            _proofImageUrl != null
                                ? 'Tap to change image'
                                : 'Tap to upload your receipt or screenshot',
                            style: TextStyle(
                              color: _kWhite.withOpacity(0.7),
                              fontSize: 9,
                              fontWeight: FontWeight.w600,
  ...
  //----------------------------------------for online payment proof upload (not fully implemented yet)----------------------------------------
  Future<void> _pickAndUploadProof() async {
    ...
    try {
      ...
      final request = http.MultipartRequest(
        'POST',
        Uri.parse('http://localhost:3006/api/orders/upload-proof'),
      );
      ...
      print('Upload response: $bodyStr');
      final body = jsonDecode(bodyStr);

      setState(() {
        _proofImageUrl = body['path'];
        _isUploading = false;
      });
    } catch (e) {
      print('Upload error: $e');
      setState(() => _isUploading = false);
      _snack('Failed to upload proof. Please try again.', error: true);
    }
  }
