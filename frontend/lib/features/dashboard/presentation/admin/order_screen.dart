...
class _OrderScreenState extends State<OrderScreen> {
  ...
  List<AdminOrder> _orders = List.from(_demoOrders);
  ...
  // ── ✅ Compact calendar popup (no full-screen takeover) ──────────────────
  Future<void> _pickDateRange(BuildContext context, Offset tapPosition) async {
    ...
    DateTimeRange? tempRange = initial;
    ...
    await showDialog(
      ...
      builder: (ctx) {
        return StatefulBuilder(builder: (ctx, setLocal) {
          ...
          int firstWeekday(int y, int m) => DateTime(y, m, 1).weekday % 7; // 0=Sun
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                ...
                                border: isToday && !selected && !start && !end
                                    ? Border.all(color: AppColors.tertiary.withOpacity(0.4))
                                    : null,
                              ),
                              child: Center(
                                child: Text('$day',
  ...
  void _clearDateRange() => setState(() { _customRange = null; _quickFilter = 'TODAY'; });
    ...
    return Column(
      ...
      children: [
        ...
        Row(children: [
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: active ? [BoxShadow(color: AppColors.tertiary.withOpacity(0.2), blurRadius: 6, offset: const Offset(0,2))] : [],
                ),
                child: Text(f, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                    letterSpacing: 0.5, color: active ? Colors.white : AppColors.tertiary)),
              ),
            );
          }),
          ...
          // ✅ Calendar trigger — uses Builder to get tap position, opens compact popup
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: _customRange != null ? AppColors.tertiary : AppColors.tertiary.withOpacity(0.25)),
                ),
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(Icons.calendar_today_outlined, size: 13,
                      color: _customRange != null ? AppColors.tertiary : AppColors.tertiary.withOpacity(0.6)),
          ...
          SizedBox(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                prefixIcon: Icon(Icons.search, size: 16, color: AppColors.tertiary.withOpacity(0.5)),
                suffixIcon: _searchQuery.isNotEmpty
                    ? GestureDetector(
                        onTap: () { _searchCtrl.clear(); setState(() => _searchQuery = ''); },
                        child: Icon(Icons.close, size: 14, color: AppColors.tertiary.withOpacity(0.5)))
                    : null,
        ...
        Row(children: [
          ...
          ...statusFilters.map((s) {
            ...
            final statusEnum = s == 'ALL' ? null
                : OrderStatus.values.firstWhere((e) => e.label == s, orElse: () => OrderStatus.pending);
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  color: active ? (statusEnum?.color ?? AppColors.tertiary).withOpacity(0.12) : Colors.transparent,
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(
                    color: active ? (statusEnum?.color ?? AppColors.tertiary).withOpacity(0.6)
                        : AppColors.tertiary.withOpacity(0.2)),
                ),
                child: Text(s, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700,
                    letterSpacing: 0.4,
                    color: active ? (statusEnum?.color ?? AppColors.tertiary) : AppColors.tertiary.withOpacity(0.6))),
              ),
            );
          }),
          const Spacer(),
  ...
  Widget _buildTable() {
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: AppColors.tertiary.withOpacity(0.07), blurRadius: 14, offset: const Offset(0,4))],
      ),
  ...
  Widget _buildTableHeader() {
    const cols = [
      _Col('DATE / TIME', flex: 2), _Col('ORDER ID', flex: 1),
      _Col('CUSTOMER',    flex: 2), _Col('ITEMS',    flex: 1),
      _Col('TYPE',        flex: 1), _Col('STATUS',   flex: 2),
      _Col('TOTAL',       flex: 1), _Col('',         flex: 2),
    ];
    return Container(
      ...
      child: Row(
        children: cols.map((c) => Expanded(
          flex: c.flex,
          child: Text(c.label, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w900,
              letterSpacing: 0.8, color: AppColors.tertiary.withOpacity(0.65))),
        )).toList(),
      ),
    );
  }
  ...
  Widget _buildRow(AdminOrder order, int index) {
    ...
    return Container(
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
        Expanded(flex: 2, child: Text(_fmt(order.datetime),
        ...
        Expanded(flex: 1, child: Align(
          ...
          child: Container(
            ...
            child: Text(order.id, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: AppColors.tertiary)),
          ),
        )),
        ...
        Expanded(flex: 2, child: Column(
          ...
          children: [
            Text(order.customer, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: Color(0xFF4a3520))),
            if (order.customerPhone != null)
              Text(order.customerPhone!, style: TextStyle(fontSize: 10, color: AppColors.tertiary.withOpacity(0.5))),
          ],
        )),

        Expanded(flex: 1, child: Text('${order.itemCount} Units',
        ...
        Expanded(flex: 1, child: Align(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: isOnline ? const Color(0xFFE8F5E9) : isPickup ? const Color(0xFFE3F2FD) : AppColors.background,
              borderRadius: BorderRadius.circular(20),
              border: Border.all(
            ...
            child: Text(order.entryType, style: TextStyle(fontSize: 9, fontWeight: FontWeight.w800,
                letterSpacing: 0.3,
        ...
        SizedBox(
            ...
            order: order,
            onChanged: (s) => _updateStatus(order, s),
          ),
        ),

        Expanded(flex: 1, child: Text('₱${order.total.toStringAsFixed(2)}',
            style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: Color(0xFF4a3520)))),
        ...
        Expanded(flex: 2, child: Align(
          ...
          child: OutlinedButton.icon(
            ...
            label: Text('VIEW RECEIPT', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700,
  ...
  Widget _buildEmptyState() {
    return Expanded(child: Center(child: Column(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
        Icon(Icons.receipt_long_outlined, size: 48, color: AppColors.tertiary.withOpacity(0.2)),
        const SizedBox(height: 12),
        Text(_searchQuery.isNotEmpty ? 'No orders match "$_searchQuery"' : 'No orders for this period',
            style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.tertiary.withOpacity(0.4))),
      ],
    )));
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// ✅ STATUS DROPDOWN — clean popup card, properly positioned
// ─────────────────────────────────────────────────────────────────────────────
...
class _StatusDropdown extends StatelessWidget {
      ...
      child: Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          Flexible(child: Text(s.label, overflow: TextOverflow.ellipsis,
              style: TextStyle(fontSize: 9, fontWeight: FontWeight.w800, letterSpacing: 0.3, color: s.color))),
  ...
  Future<void> _showStatusMenu(BuildContext context, OrderStatus current) async {
    // Get the render box of the tapped widget to position the menu below it
    ...
    await showDialog(
      ...
      builder: (ctx) {
        return Stack(children: [
          ...
          // ✅ Popup card — positioned directly below the status pill
          Positioned(
            left: offset.dx,
                      ...
                      return GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: Row(children: [
                            ...
                            const SizedBox(width: 10),
                            Expanded(child: Text(status.label, style: TextStyle(
                                fontSize: 12,
                                fontWeight: isActive ? FontWeight.w800 : FontWeight.w500,
                                color: isActive ? status.color : const Color(0xFF4a3520)))),
                            if (isActive)
                              Icon(Icons.check_rounded, size: 14, color: status.color),
                          ]),
                        ),
                      );
                    }),
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        width: 32, height: 32,
        decoration: BoxDecoration(
          color: onTap != null ? AppColors.tertiary.withOpacity(0.08) : Colors.transparent,
          borderRadius: BorderRadius.circular(8),
        ),
        child: Icon(icon, size: 18,
            color: onTap != null ? AppColors.tertiary : AppColors.tertiary.withOpacity(0.2)),
      ),
    );
  }
}
            ...
            children: [
              Row(children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('ORDER RECEIPT', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900,
                      letterSpacing: 0.5, color: AppColors.tertiary)),
                  const SizedBox(height: 2),
                  Text(_fmt(order.datetime), style: TextStyle(fontSize: 11, color: AppColors.tertiary.withOpacity(0.5))),
                ])),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                  decoration: BoxDecoration(color: order.status.bg, borderRadius: BorderRadius.circular(20),
                      border: Border.all(color: order.status.color.withOpacity(0.4))),
                  child: Text(order.status.label, style: TextStyle(fontSize: 9, fontWeight: FontWeight.w900, color: order.status.color)),
                ),
                const SizedBox(width: 10),
                GestureDetector(
                  ...
                  child: Container(
                    padding: const EdgeInsets.all(6),
                    decoration: BoxDecoration(shape: BoxShape.circle, color: AppColors.tertiary.withOpacity(0.08)),
              ...
              if (order.customerPhone != null) _ReceiptRow(label: 'Phone', value: order.customerPhone!),
              _ReceiptRow(label: 'Entry Type', value: order.entryType),
              if (order.note != null) _ReceiptRow(label: 'Note', value: order.note!),
              ...
              Flexible(child: ListView(
                ...
                children: order.items.isEmpty
                    ? [Text('No item details.', style: TextStyle(fontSize: 12, color: AppColors.tertiary.withOpacity(0.4)))]
                    : order.items.map((item) => Padding(
                        padding: const EdgeInsets.only(bottom: 8),
                        child: Row(children: [
                          Text('${item.qty}×', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: AppColors.tertiary)),
                          const SizedBox(width: 8),
                          Expanded(child: Text(item.name, style: TextStyle(fontSize: 12, color: AppColors.tertiary.withOpacity(0.85)))),
                          Text('₱${item.subtotal.toStringAsFixed(2)}',
                              style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: Color(0xFF4a3520))),
                        ]),
                      )).toList(),
              )),
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text('TOTAL', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w900, color: AppColors.tertiary)),
                Text('₱${order.total.toStringAsFixed(2)}',
                    style: const TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: Color(0xFF4a3520))),
              ]),
              const SizedBox(height: 20),
              ...
              Row(children: [
                Expanded(child: OutlinedButton.icon(
                  ...
                  label: Text('PRINT', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.tertiary)),
                  style: OutlinedButton.styleFrom(
                    side: BorderSide(color: AppColors.tertiary.withOpacity(0.4)),
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
        Text(label, style: TextStyle(fontSize: 11, color: AppColors.tertiary.withOpacity(0.5))),
        Flexible(child: Text(value, textAlign: TextAlign.right,
            style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.tertiary))),
      ]),
    );
  }
}
